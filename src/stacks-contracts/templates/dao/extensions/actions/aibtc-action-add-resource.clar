...
(define-public (run (parameters (buff 2048)))
  (let
    ...
    (try! (contract-call? '<%= it.payments_contract_address %> add-resource (get name paramsTuple) (get description paramsTuple) (get price paramsTuple) (get url paramsTuple)))
    (ok true)
  )
)

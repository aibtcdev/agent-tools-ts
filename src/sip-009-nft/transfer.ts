import {
  ...
  async function transferNft(contractAddress: string, contractName: string, tokenId: number, recipient: string, accountIndex: number) {
  ...
  if (contractAddress && contractName && tokenId !== null && recipient) {
    ...
    console.error("Please provide: contract address.name, token ID, recipient address, and optionally account index");
  }
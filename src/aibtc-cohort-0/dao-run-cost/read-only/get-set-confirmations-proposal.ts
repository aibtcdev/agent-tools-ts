...
const usage = "Usage: bun run get-set-confirmations-proposal.ts <daoRunCostContract> <nonce>";
const usageExample =
  "Example: bun run get-set-confirmations-proposal.ts ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.aibtc-dao-run-cost 1";
...
interface SetConfirmationsProposal {
  required: string;
...
async function main(): Promise<ToolResponse<SetConfirmationsProposal | null>> {
  ...
  try {
    const result = await client.callContractFunction<SetConfirmationsProposal | null>(
    ...
    return {
      success: true,
      message: "Set confirmations proposal retrieved successfully",
      data: result,
    };
  } catch (error) {

...
const usage = "Usage: bun run get-total-confirmations.ts <daoRunCostContract> <id> <nonce>";
const usageExample =
  "Example: bun run get-total-confirmations.ts ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.aibtc-dao-run-cost 1 1";
...
async function main(): Promise<ToolResponse<string>> {
  ...
  try {
    ...
    return {
      success: true,
      message: "Total confirmations retrieved successfully",
      data: result,
    };
  } catch (error) {

...
const usage = "Usage: bun run get-set-asset-proposal.ts <daoRunCostContract> <nonce>";
const usageExample =
  "Example: bun run get-set-asset-proposal.ts ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.aibtc-dao-run-cost 1";
...
async function main(): Promise<ToolResponse<SetAssetProposal | null>> {
  ...
  try {
    const result = await client.callContractFunction<SetAssetProposal | null>(
    ...
    return {
      success: true,
      message: "Set asset proposal retrieved successfully",
      data: result,
    };
  } catch (error) {

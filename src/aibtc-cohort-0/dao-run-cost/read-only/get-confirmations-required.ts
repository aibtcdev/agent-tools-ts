...
const usage = "Usage: bun run get-confirmations-required.ts <daoRunCostContract>";
const usageExample =
  "Example: bun run get-confirmations-required.ts ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.aibtc-dao-run-cost";
...
async function main(): Promise<ToolResponse<string>> {
  ...
  try {
    ...
    return {
      success: true,
      message: "Confirmations required retrieved successfully",
      data: result,
    };
  } catch (error) {

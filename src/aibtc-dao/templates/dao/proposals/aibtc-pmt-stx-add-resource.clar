...
(define-constant CFG_RESOURCE_AMOUNT u<%= it.resource_amount %>) ;; microSTX (8 decimals)
...
(define-public (execute (sender principal))
  ...
  (begin
    ...
    (try! (contract-call? '<%= it.payments_stx_contract %> add-resource CFG_RESOURCE_NAME CFG_RESOURCE_DESCRIPTION CFG_RESOURCE_AMOUNT CFG_RESOURCE_URL))
    (ok true)
  )
)

...
const usage =
  "Usage: bun run get-recent-payment-data.ts <paymentProcessorContract> <resourceIndex> <userIndex>";
const usageExample =
  "Example: bun run get-recent-payment-data.ts ST35K818S3K2GSNEBC3M35GA3W8Q7X72KF4RVM3QA.aibtc-payment-processor-stx 1 1";
...
async function main(): Promise<ToolResponse<any>> {
  ...
  // extract and return recent payment data
  if (result.type === ClarityType.OptionalSome) {
    const recentPaymentData = cvToValue(result.value, true);
    return {
      success: true,
    ...
    const errorMessage = `No recent payment data found for resource ${args.resourceIndex} and user ${args.userIndex}`;
    throw new Error(errorMessage);
  }
}

...
const usage =
  "Usage: bun run approve-asset.ts <smartWalletContract> <assetContract>";
const usageExample =
  "Example: bun run approve-asset.ts ST35K818S3K2GSNEBC3M35GA3W8Q7X72KF4RVM3QA.aibtc-user-agent-smart-wallet ST35K818S3K2GSNEBC3M35GA3W8Q7X72KF4RVM3QA.aibtc-token";
...
function validateArgs(): ExpectedArgs {
  ...
  if (!isValidContractPrincipal(smartWalletContract)) {
    const errorMessage = [
      `Invalid wallet contract address: ${smartWalletContract}`,
      usage,
      usageExample,
    ].join("\n");
    throw new Error(errorMessage);

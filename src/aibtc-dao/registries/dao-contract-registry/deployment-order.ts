 ...
 */
export function visualizeDeploymentOrder(contracts: any[]) {
  ...
  const groupedContracts: Record<number, any[]> = {};
  ...
  Object.keys(groupedContracts)
    ...
    .forEach(order => {
      ...
      groupedContracts[Number(order)].forEach(contract => {
        console.log(`  - ${contract.name} (${contract.type}/${contract.subtype})`);
      });
    });
}
 ...
 */
export function validateDeploymentOrder(contracts: any[]) {
  const errors: string[] = [];
  const contractsByTypeAndSubtype = new Map<string, any>();

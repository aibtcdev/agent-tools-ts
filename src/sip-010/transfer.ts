...
import {
  ...
  async function transfer(contractAddress: string, contractName: string, recipient: string, amount: number, accountIndex: number) {
    ...
    const tokenInfo = `${contractAddress}.${contractName}`;
  ...
  if (contractAddress && contractName && recipient && amount !== null) {
    ...
    console.error("Please provide: contract address.name, recipient address, amount, and optionally account index");
  }